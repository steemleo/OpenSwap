...
```bash
npm run brand         # = node scripts/gen-brand.mjs   (zero dependencies)
npm run brand:social  # = node scripts/gen-social-png.mjs  (needs Chrome)
```
...
`brand` is dependency-free because it only ever encodes an integer-scaled pixel
grid. The social card carries live text, so rasterizing it needs a real font
shaper — `brand:social` shells out to an installed Chrome (override with
`CHROME_PATH`) rather than adding a heavyweight dependency for one asset. It is
deterministic: same SVG in, byte-identical PNG out. Rerun it whenever the card
...
| `social-card.svg` | 1280×640 social preview (vector source) |
| `png/social-card.png` | 2560×1280 (2× for retina) — this is the file you upload under GitHub → Settings → Social preview, which accepts PNG/JPG/GIF but **not** SVG. Generated by `npm run brand:social`. |
